...
//        }
    ...
    public void deleteCategoryRelation(CategoryRelationDTO relation) {
        ...
        // Delete relation
        categoryCategoryRepository.delete(relations.getFirst());
    }
...
//    }
    ...
    /**
     * Check if two categories have common descendants
     */
    private boolean hasCommonDescendants(Long parentId, Long childId) {
        Set<Long> parentDescendants = getAllDescendants(parentId);
        Set<Long> childDescendants = getAllDescendants(childId);

        // If they have common descendants, creating parent->child relation would create diamond
        parentDescendants.retainAll(childDescendants);
        return !parentDescendants.isEmpty();
    }

    /**
     * Get all descendants of a category
     */
    private Set<Long> getAllDescendants(Long categoryId) {
        Set<Long> descendants = new HashSet<>();
        Set<Long> visited = new HashSet<>();
        collectDescendants(categoryId, descendants, visited);
        return descendants;
    }
     ...
     */
    private void collectDescendants(Long categoryId, Set<Long> descendants, Set<Long> visited) {
        if (visited.contains(categoryId)) {
            return; // Cycle protection
        }
        visited.add(categoryId);

        List<Long> children = categoryCategoryRepository.findChildIds(categoryId);
        for (Long childId : children) {
            descendants.add(childId);
            collectDescendants(childId, descendants, visited);
        }
    }

    /**
     * Check if two categories have common ancestors
     */
    private boolean hasCommonAncestors(Long parentId, Long childId) {
        Set<Long> parentAncestors = getAllAncestors(parentId);
        Set<Long> childAncestors = getAllAncestors(childId);

        // If they have common ancestors, creating parent->child relation would create diamond
        parentAncestors.retainAll(childAncestors);
        return !parentAncestors.isEmpty();
    }

    /**
     * Get all ancestors of a category
     */
    private Set<Long> getAllAncestors(Long categoryId) {
        Set<Long> ancestors = new HashSet<>();
        Set<Long> visited = new HashSet<>();
        collectAncestors(categoryId, ancestors, visited);
        return ancestors;
    }
     ...
     */
    private void collectAncestors(Long categoryId, Set<Long> ancestors, Set<Long> visited) {
        if (visited.contains(categoryId)) {
            return; // Cycle protection
        }
        visited.add(categoryId);

        List<Long> parents = categoryCategoryRepository.findParentIds(categoryId);
        for (Long parentId : parents) {
