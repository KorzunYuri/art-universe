...
@Tag("integration")
public class ArtistBindingRepositoryTest extends JpaOnlyTest {
    ...
    void shouldReturnBoundArtistsWithNames() {
        // given
        Artist reference = em.persist(Artist.builder().name("Radiohead").build());
        em.flush();
        ...
        assertThat(result.getReferenceName()).isEqualTo("Radiohead");
    }

    @Test
    void shouldHandleMultipleBindings() {
        // given
        Artist reference1 = em.persist(Artist.builder().name("Radiohead").build());
        Artist reference2 = em.persist(Artist.builder().name("Coldplay").build());
        em.flush();
