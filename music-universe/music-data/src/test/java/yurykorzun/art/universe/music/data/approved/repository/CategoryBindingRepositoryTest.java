...
public class CategoryBindingRepositoryTest extends JpaOnlyTest {
    ...
    void shouldReturnBoundCategoriesWithNames() {
        // given
        Category reference = em.persist(Category.builder().name("Rock").build());
        em.flush();
        ...
        assertThat(result.getReferenceId()).isEqualTo(reference.getId());
        assertThat(result.getReferenceName()).isEqualTo("Rock");
    }

    @Test
    void shouldHandleMultipleBindings() {
        // given
        Category reference1 = em.persist(Category.builder().name("Rock").build());
        Category reference2 = em.persist(Category.builder().name("Jazz").build());
        em.flush();
        ...
        assertThat(results).extracting(BoundEntityProjection::getReferenceName)
    ...
    void shouldReturnSingleBoundCategory() {
        // given
        Category reference = em.persist(Category.builder().name("Electronic").build());
        em.flush();
        ...
        assertThat(result.getDataSource()).isEqualTo(DataSource.LASTFM);
        assertThat(result.getReferenceId()).isEqualTo(reference.getId());
        assertThat(result.getReferenceName()).isEqualTo("Electronic");
    }
