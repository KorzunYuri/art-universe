...
class LastfmArtistTopTagsApiCallGeneratorTest extends JpaOnlyTest {
    ...
    void givenArtist_whenCreateApiCallsCalled_createsSnapshotsAndArtistTopTagsApiCalls() {
        ...
        when(snapshotService.getOrCreateSnapshotFor(eq(generator.getApiCallType()), any(LastfmArtist.class)))
        ...
        for (LastfmArtist artist : unprocessedArtists) {
            verify(snapshotService).getOrCreateSnapshotFor(eq(generator.getApiCallType()), eq(artist));
        }
        ArgumentCaptor<List<Long>> snapshotCreatedCountIncCaptor = ArgumentCaptor.forClass(List.class);
        verify(snapshotService).incCreatedCount(snapshotCreatedCountIncCaptor.capture());
        ...
        capturedCalls.forEach(request -> {
            ...
            assertNotNull(artistName, "'artist' must be present in api call parameters'");
