...
| MURAW_LASTFM_DB_PORT      | Database port                                     | No           | 9999          |
| MURAW_LASTFM_API_KEY      | API key obtained from Lastfm for public API calls | Yes          |               |
| MURAW_LASTFM_PORT         | Port to run the app's web server on               | No           | 8081          |
...
The most convenient way is to create a dev environment using Docker.
...
For the first time, create _.env_ file under _src/main/resources/docker/compose/_ with the following variables at least:
```
...
```
get-content music-universe/music-data-raw-lastfm/src/main/resources/docker/compose/.env | foreach {
  $name, $value = $_.split('=')
