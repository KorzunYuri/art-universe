-- Update mtnc_cleanup_entity function to add deleted entities to blacklist before deletion
...
BEGIN
    ...
    v_sql := format(
            'INSERT INTO tmp_entities_to_delete (id, url)
             SELECT id, url
             FROM %I
    ...
    CREATE INDEX ON tmp_entities_to_delete (url);
    ...
    IF in_dry_run THEN
        ...
        -- 0) Blacklist additions
        SELECT COUNT(*) INTO v_deleted
        FROM tmp_entities_to_delete
        WHERE url IS NOT NULL AND url != '';

        CALL mtnc_cleanup_history_add_message(v_run_id, format('blacklist_entity_url: %s URLs will be added to blacklist', v_deleted));

        -- 1A) attribute_history_current
        SELECT COUNT(*) INTO v_total FROM attribute_history_current;

        SELECT COUNT(*) INTO v_deleted
        FROM attribute_history_current ah
        ...
        CALL mtnc_cleanup_history_add_message(v_run_id, format('attribute_history_current: to remove %s out of %s (%s%%) (entity records)',
        ...
        CALL mtnc_cleanup_history_add_message(v_run_id, format('attribute_history_current: to remove %s out of %s (%s%%) (records with entity as scope)',
                                                               v_deleted, v_total, v_percentage));

        -- 1b) attribute_history_archive
        SELECT COUNT(*) INTO v_total FROM attribute_history_archive;

        SELECT COUNT(*) INTO v_deleted
        FROM attribute_history_archive ah
        JOIN tmp_entities_to_delete tmp
            ON      ah.entity_type  = in_entity_type
                AND ah.entity_id    = tmp.id;
        ...
        CALL mtnc_cleanup_history_add_message(v_run_id, format('attribute_history_archive: to remove %s out of %s (%s%%) (entity records)',
                                                               v_deleted, v_total, v_percentage));
        ...
        CALL mtnc_cleanup_history_add_message(v_run_id, format('attribute_history_archive: to remove %s out of %s (%s%%) (records with entity as scope)',
                                                               v_deleted, v_total, v_percentage));
    ...
    ELSE
        ...
        -- 0) Add entities to blacklist BEFORE deletion
        CALL mtnc_cleanup_history_add_message(v_run_id, 'adding entities to blacklist started');

        INSERT INTO blacklist_entity_url (entity_type, url)
        SELECT in_entity_type, tmp.url
        FROM tmp_entities_to_delete tmp
        WHERE tmp.url IS NOT NULL
          AND tmp.url != ''
        ON CONFLICT (entity_type, url) DO NOTHING;

        GET DIAGNOSTICS v_deleted = ROW_COUNT;
        CALL mtnc_cleanup_history_add_message(v_run_id, format('blacklist_entity_url: added %s URLs to blacklist', v_deleted));
        ...
        -- 1a) attribute_history_current
        CALL mtnc_cleanup_history_add_message(v_run_id, 'cleanup attribute_history_current started');

        DELETE FROM attribute_history_current ah
            USING tmp_entities_to_delete tmp
        WHERE ah.entity_type = in_entity_type
          AND ah.entity_id = tmp.id;

        GET DIAGNOSTICS v_deleted = ROW_COUNT;
        CALL mtnc_cleanup_history_add_message(v_run_id, format('attribute_history_current: removed %s records', v_deleted));
        ...
        GET DIAGNOSTICS v_deleted = ROW_COUNT;
        CALL mtnc_cleanup_history_add_message(v_run_id, format('attribute_history_current: removed %s records with entity as scope', v_deleted));

        -- 1b) attribute_history_archive
        CALL mtnc_cleanup_history_add_message(v_run_id, 'cleanup attribute_history_archive started');

        DELETE FROM attribute_history_archive ah
            USING tmp_entities_to_delete tmp
        WHERE ah.entity_type = in_entity_type
          AND ah.entity_id = tmp.id;

        GET DIAGNOSTICS v_deleted = ROW_COUNT;
        CALL mtnc_cleanup_history_add_message(v_run_id, format('attribute_history_archive: removed %s records', v_deleted));
        ...
        GET DIAGNOSTICS v_deleted = ROW_COUNT;
        CALL mtnc_cleanup_history_add_message(v_run_id, format('attribute_history_archive: removed %s records with entity as scope', v_deleted));
