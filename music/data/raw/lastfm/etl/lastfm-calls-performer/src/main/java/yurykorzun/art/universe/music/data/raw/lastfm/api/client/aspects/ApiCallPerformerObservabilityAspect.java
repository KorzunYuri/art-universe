package yurykorzun.art.universe.music.data.raw.lastfm.api.client.aspects;
...
import yurykorzun.art.universe.music.data.raw.lastfm.api.client.service.impl.LastfmApiCallServiceImpl;

/**
 * AOP aspect that creates timing metrics for API call execution operations.
 *
 * <p>Uses @Timed (not @Observed) at the moment as I am only interested in a single call operation duration</p>
 *
 * @see LastfmApiCallServiceImpl
 */
@Aspect
@Component
@Slf4j
public class ApiCallPerformerObservabilityAspect {
    ...
    /**
     * Intercepts all executions of {@link LastfmApiCallServiceImpl#makeApiCall(LastfmApiCall)}
     ...
     */
    @Around("execution(* yurykorzun.art.universe.music.data.raw.lastfm.api.client.service.impl.LastfmApiCallServiceImpl.makeApiCall(..)) && args(call)")
    public Object timeExecution(ProceedingJoinPoint joinPoint, LastfmApiCall call) throws Throwable {
    ...
    private Timer createTimer(String apiCallType, String status) {
        return Timer.builder("music.data.raw.lastfm.api.call.perform")
                .description("Lastfm API call execution duration: single")
