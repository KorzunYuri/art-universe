...
import yurykorzun.art.universe.music.data.raw.lastfm.domain.dto.*;
...
class LastfmTagControllerTest {
    ...
    void getTagById_shouldReturnTagWhenFound() {
        ...
        when(tagService.findById(tagId)).thenReturn(responseDto);
    ...
    void getEntityTags_shouldReturnTagsForValidEntityTypeName() {
        ...
        assertEquals("electronic", response.getFirst().name());
        assertEquals(ApprovalStatus.APPROVED.getCode(), response.getFirst().relationApprovalStatus());
        assertEquals(ApprovalStatus.APPROVED.getCode(), response.getFirst().tagApprovalStatus());
        assertEquals(ApprovalStatus.PENDING.getCode(), response.getFirst().entityApprovalStatus());
        assertEquals(50, response.getFirst().usageCount());
    }
    ...
    void lookupTags_shouldHandleNullLimit() {
        ...
        assertEquals("jazz", result.getFirst().getName());
    }
