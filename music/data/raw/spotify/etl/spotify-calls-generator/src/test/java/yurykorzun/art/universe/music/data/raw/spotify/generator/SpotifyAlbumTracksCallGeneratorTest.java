...
class SpotifyAlbumTracksCallGeneratorTest {
    ...
    @InjectMocks
    private SpotifyAlbumTracksCallGenerator generator;

    @Test
    void createApiCalls_shouldSkip_whenNoAlbumsNeedCalls() {
        when(entityService.findAllWithoutActiveCalls(SpotifyAlbum.class, SpotifyApiCallType.ALBUM_TRACKS))
            .thenReturn(List.of());
    ...
    void createApiCalls_shouldCreateCallForEachAlbum() {
        ...
        when(configPropertyHolder.getInt(SpotifyGeneratorProperty.DUE_DURATION_ALBUM_TRACKS)).thenReturn(7);
        ...
        assertThat(requests.get(0).getType()).isEqualTo(SpotifyApiCallType.ALBUM_TRACKS);
    ...
    void getApiCallType_shouldReturnAlbumTracks() {
        assertThat(generator.getApiCallType()).isEqualTo(SpotifyApiCallType.ALBUM_TRACKS);
    }
}
