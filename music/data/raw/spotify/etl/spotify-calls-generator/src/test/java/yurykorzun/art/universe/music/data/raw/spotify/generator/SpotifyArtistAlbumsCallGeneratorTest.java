...
class SpotifyArtistAlbumsCallGeneratorTest {
    ...
    @InjectMocks
    private SpotifyArtistAlbumsCallGenerator generator;

    @Test
    void createApiCalls_shouldSkip_whenNoArtistsNeedCalls() {
        when(entityService.findAllWithoutActiveCalls(SpotifyArtist.class, SpotifyApiCallType.ARTIST_ALBUMS))
            .thenReturn(List.of());
    ...
    void createApiCalls_shouldCreateCallForEachArtist() {
        ...
        when(configPropertyHolder.getInt(SpotifyGeneratorProperty.DUE_DURATION_ARTIST_ALBUMS)).thenReturn(7);
        ...
        assertThat(requests.get(0).getType()).isEqualTo(SpotifyApiCallType.ARTIST_ALBUMS);
        assertThat(requests.get(0).getEntityType()).isEqualTo(SpotifyEntityType.ARTIST);
    ...
    void getApiCallType_shouldReturnArtistAlbums() {
        assertThat(generator.getApiCallType()).isEqualTo(SpotifyApiCallType.ARTIST_ALBUMS);
    }
}
