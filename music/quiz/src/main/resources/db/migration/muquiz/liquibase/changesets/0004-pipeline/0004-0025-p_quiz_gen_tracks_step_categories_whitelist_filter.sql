-- Include only whitelisted artist categories with weights
-- Extra inputs: whitelist_schema, whitelist_table (category_id, weight)
...
BEGIN
    ...
    -- Create output table with whitelisted categories and chance calculation
    IF has_chance_column THEN
        EXECUTE format('
            CREATE TABLE %I.%I AS
            SELECT 
                %s,
                inp.chance * wl.weight as chance
            FROM %I.%I inp
            JOIN mu_view.v_artist_category vac ON inp.primary_artist_id = vac.artist_id
            JOIN (
                SELECT DISTINCT vch.id as category_id, wl.weight
                FROM %I.%I wl
                JOIN mu_view.v_category_hierarchy vch ON (
                    vch.id = wl.category_id OR vch.parent_id = wl.category_id
                )
            ) wl ON vac.category_id = wl.category_id
            WHERE inp.chance * wl.weight > 0
        ', target_schema, output_table_name, columns_list, input_schema, input_table, whitelist_schema, whitelist_table);
    ELSE
        EXECUTE format('
            CREATE TABLE %I.%I AS
            SELECT 
