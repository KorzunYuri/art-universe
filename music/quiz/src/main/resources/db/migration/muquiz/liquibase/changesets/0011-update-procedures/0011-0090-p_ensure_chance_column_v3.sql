-- Update p_ensure_chance_column to handle re-execution by dropping existing table before creation
...
BEGIN
    ...
    IF is_view THEN
        ...
        -- Drop existing table if it exists (for re-execution scenarios)
        EXECUTE format('DROP TABLE IF EXISTS mu_quiz_stg.%I', new_table_name);
