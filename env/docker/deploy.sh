...
if [ $? -eq 0 ]; then
    ...
    echo "  Core:"
    echo "  - Config Service: http://localhost:${AU_CONFIG_APP_EXTERNAL_PORT}"
    echo "  - Auth Service: http://localhost:${AU_AUTH_APP_EXTERNAL_PORT}"
    echo "  LastFM:"
    ...
    echo "  Spotify:"
    echo "  - Spotify REST API: http://localhost:${MURAW_SPOTIFY_REST_API_EXTERNAL_PORT}"
    echo "  - Spotify Calls Generator: actuator http://localhost:${MURAW_SPOTIFY_CALLS_GENERATOR_ACTUATOR_EXTERNAL_PORT}"
    echo "  - Spotify Calls Performer: actuator http://localhost:${MURAW_SPOTIFY_CALLS_PERFORMER_ACTUATOR_EXTERNAL_PORT}"
    echo "  - Spotify Response Parser: actuator http://localhost:${MURAW_SPOTIFY_RESPONSE_PARSER_ACTUATOR_EXTERNAL_PORT}"
    echo "  - Spotify Staging Applicator: actuator http://localhost:${MURAW_SPOTIFY_STAGING_APPLICATOR_ACTUATOR_EXTERNAL_PORT}"
    echo "  Apps:"
    echo "  - Music Data: http://localhost:${MU_DATA_APP_EXTERNAL_PORT}"
    echo "  - Music Quiz: http://localhost:${MU_QUIZ_APP_EXTERNAL_PORT}"
    echo "  - Art Data: http://localhost:${ART_DATA_APP_EXTERNAL_PORT}"
    echo "  Frontend:"
