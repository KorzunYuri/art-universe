...
#   MURAW_SPOTIFY_DB_WRITER_PASSWORD        - spotify dm user password
#   MURAW_SPOTIFY_DB_READER_PASSWORD        - spotify reader user password
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "art_universe" <<-EOSQL
    ...
    -- =============================================================================
    -- SPOTIFY RAW DATA SCHEMAS
    -- =============================================================================

    -- Spotify data management user (write access)
    CREATE USER mu_raw_spotify_dm WITH LOGIN PASSWORD '$MURAW_SPOTIFY_DB_WRITER_PASSWORD';

    -- Spotify read-only user (for replica reads)
    ...
    -- Set search paths for spotify users
