...
# Art Universe - Cleanup Orchestrator
# Usage: ./scripts/cleanup.sh <docker|k8s> <environment>
#
...
#
# Examples:
#   ./scripts/cleanup.sh docker prod
#   ./scripts/cleanup.sh k8s local
...
case "$PLATFORM" in
    docker)
        exec "$PROJECT_ROOT/env/docker/cleanup.sh" "$ENVIRONMENT"
        ;;
    k8s)
        exec "$PROJECT_ROOT/env/k8s/scripts/cleanup.sh" "$ENVIRONMENT"
        ;;
esac
